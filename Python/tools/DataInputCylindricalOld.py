...
def decode_csv(line):
    ...
    image_ch0 = (tf.cast(tf.image.decode_png(image_str_ch0, channels=4, dtype=tf.uint16), dtype=tf.float32) - 32768) / 200
    image_str_ch1 = tf.read_file(vec[1])
    image_ch1 = (tf.cast(tf.image.decode_png(image_str_ch1, channels=4, dtype=tf.uint16), dtype=tf.float32) - 32768) / 200
    ...
    return tf.reshape(tf.stack([image_ch0[:,:,0:3], image_ch1[:,:,0:3]], axis=2), [45,628,6]), tf.slice(image_gt, [0, 0, 0], [45,628,2])
