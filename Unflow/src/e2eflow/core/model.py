...
class PWCNet(object):
    def __init__(self, num_levels = 6, search_range = 4, warp_type = 'bilinear',
        ...
        self.of_estimators = [OpticalFlowEstimator(self.batch_norm, name = 'optflow_' + str(l))
        ...
        if self.context is 'all':
            self.context_nets = [ContextNetwork(name = 'context_' + str(l)) for l in range(self.num_levels)]
        else:
        ...
        with tf.variable_scope(self.name) as vs:
            ...
            for l, (feature_0, feature_1) in enumerate(zip(pyramid_0, pyramid_1)):
                ...
                if l == self.output_level:
                    ...
                    print('Finally upscale flow by' + upscale)
