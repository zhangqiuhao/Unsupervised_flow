...
def flownet(im1, im2, layer, flownet_spec='S', full_resolution=False, train_all=False,
            backward_flow=False):
...
def flownet_c(conv3_a, conv3_b, conv2_a, channel_mult=1, full_res=False):
    ...
    with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
        ...
        conv5_1 = slim.conv2d(conv4_1, int(512 * m), 3, stride=1, scope='conv5_1')
        conv6_1 = slim.conv2d(conv5_1, int(1024 * m), 3, stride=1, scope='conv6_1')
