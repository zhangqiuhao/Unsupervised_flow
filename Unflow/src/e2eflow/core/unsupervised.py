...
def unsupervised_loss(batch, start_iter, params, normalization=None, augment=True,
                      return_flow=False, evaluate=False, eval_mask=False):
    params.get('')
    ...
    if not evaluate:
        ...
        for loss in LOSSES + ['occ']:
            combined_losses[loss] = 0.0
        if params.get('motion_mask'):
            print('Motion Mask activated')
            motion_loss, motion_mask_loss, motion_fw, motion_mask_fw = \
                compute_motion_loss(im1_s, flows_fw[0]*final_flow_scale, im2_s, flows_bw[0]*final_flow_scale, num_layer,
                                    border_mask=mask_s if params.get('border_mask')
                                    else None)
        ...
        for i, flow_pair in flow_enum:
            ...
            with tf.variable_scope(layer_name):
                ...
                if mask_type is '':
                    mask_type = 'binary'
                assert mask_type in ['linear', 'binary', 'None']
    ...
    if eval_mask:
        if params.get('motion_mask'):
            ...
            return motion_mask_fw, final_flow_fw, final_flow_bw, im1'''
